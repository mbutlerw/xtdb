(ns xtdb.xtql
  ...
  (:import (clojure.lang MapEntry)
           ...
           (xtdb.operator PreparedQuery)
           (xtdb.query Expr$Call Expr$LogicVar Expr$Obj Expr$Long Expr$Subquery Query$From Query$Return
    ...
    (when-not (= (count provided-vars) 1)
      (throw (err/illegal-arg
              ...
              {:subquery subquery :provided-vars provided-vars
               ::err/message "Scalar subquery must only contain a single column"})))
    (if (seq args)
      [:apply
       :single-join
