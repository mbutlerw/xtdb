  ...
  (util/with-open [node (xtn/start-node {})]
    ...
    (t/is (= #{{:last-updated "tx2",
             ...
             (set (tu/query-ra '[:scan {:table foo, :for-system-time :all-time}
                                 [{xt$system_from (< xt$system_from #time/zoned-date-time "3002-01-01T00:00Z")}
                                  {xt$system_to (> xt$system_to #time/zoned-date-time "2999-01-01T00:00Z")}
                                  xt$valid_from
                                  xt$valid_to
                                  last_updated]]
...
(deftest live-hash-trie-branches-get-expanded-3247
  (xt/submit-tx tu/*node* [[:put-docs :foo {:xt/id 1 :bar "biff"}]])

  (t/is (= 11
           (tu/query-ra
            '[:left-outer-join
             [{c.2/oid d.3/objoid} (= d.3/objsubid 0) (= d.3/classoid 1259)]
             [:rename
              c.2
              [:scan
               {:table pg_catalog/pg_class, :for-valid-time :all-time}
               [relkind oid relnamespace relname #_{relname (like relname "'%")}]]]
             [:rename
              d.3
              [:scan
               {:table pg_catalog/pg_description, :for-valid-time :all-time}
               [objsubid description objoid classoid]]]]
            #_'[:order-by
              [[table_type {:direction :asc, :null-ordering :nulls-last}]
               [table_schem {:direction :asc, :null-ordering :nulls-last}]
               [table_name {:direction :asc, :null-ordering :nulls-last}]]
              [:project
               [{table_cat nil}
                {table_schem n.1/nspname}
                {table_name c.2/relname}
                {table_type
                 (case
                     (or
                      (like-regex n.1/nspname "^pg_" "")
                      (= n.1/nspname "information_schema"))
                   true
                   (cond
                     (or
                      (= n.1/nspname "pg_catalog")
                      (= n.1/nspname "information_schema"))
                     (case
                         c.2/relkind
                       "r"
                    ...
                    (or
                     (or
                      ...
                      (and
                       (and
                        (= c.2/relkind "r")
                        (not (like-regex n.1/nspname "^pg_" "")))
                       (<> n.1/nspname "information_schema")))
                     (and
                      (and
                       (= c.2/relkind "p")
                      ...
                      (<> n.1/nspname "information_schema")))
                    (and
                     (and (= c.2/relkind "v") (<> n.1/nspname "pg_catalog"))
                     (<> n.1/nspname "information_schema")))
                   (= c.2/relkind "f"))
                  (= c.2/relkind "m"))]
                [[:rename
                  n.1
                  [:scan
                   {:table pg_catalog/pg_namespace, :for-valid-time :all-time}
                   [oid {nspname (like nspname "'\"public\"")}]]]
                 [:left-outer-join
                  [{c.2/oid d.3/objoid} (= d.3/objsubid 0) (= d.3/classoid 1259)]
                  [:rename
                   c.2
                   [:scan
                    {:table pg_catalog/pg_class, :for-valid-time :all-time}
                    [relkind oid relnamespace {relname (like relname "'%")}]]]
                  [:rename
                   d.3
                   [:scan
                    {:table pg_catalog/pg_description, :for-valid-time :all-time}
