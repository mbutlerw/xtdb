...
(t/deftest test-explain-plan-sql
  (t/is (= [{:plan
             "[:rename {x1 xt$id, x2 foo} [:project [x1 x2] [:rename {xt$id x1, foo x2, a x3, b x4} [:select (= (+ a b) 12) [:scan {:table users} [xt$id foo a b]]]]]]"}]
           (xt/q tu/*node*
...
(deftest test-plan-query-cache
  (let [query-src ^IRaQuerySource (tu/component :xtdb.query/ra-query-source)
